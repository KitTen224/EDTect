...
import { usePersistentTravelForm } from '@/hooks/usePersistentTravelForm';
...
export default function JapanTravelForm({ onSubmit, isLoading = false }: JapanTravelFormProps) {
    // Use persistent form state hook
    const {
        step,
        selectedRegions,
        selectedStyles,
        ...
        storageInfo,
        isMounted
    } = usePersistentTravelForm({
        onSaveError: (error) => console.warn('Form auto-save failed:', error),
        onLoadError: (error) => console.warn('Form restore failed:', error)
    });
    ...
    // Show loading state only after mounted and while restoring data
    if (isMounted && isRestoring) {
        return (
            <div className="min-h-screen bg-gradient-to-br from-red-50 via-white to-pink-50 flex items-center justify-center">
                <div className="text-center">
                    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-red-500 mx-auto mb-4"></div>
                    <p className="text-gray-600">Loading travel form...</p>
    ...
    // On server-side or before mount, show the form directly (no loading state)
                        ...
                        <div className="flex items-center">
                            ...
                            <div>
                                <p className="text-sm text-yellow-800">
                                    Auto-save is currently unavailable. Your progress may be lost if you refresh the page.
                                </p>
                                <p className="text-xs text-yellow-600 mt-1">
                                    Storage: {storageInfo.adapterType} | Available: {storageInfo.isAvailable ? 'Yes' : 'No'}
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <AnimatePresence mode="wait">
                    ...
                    {step === 'summary' && canProceed && (
                        ...
                        >
                            ...
                            <div className="text-center space-x-4 flex flex-wrap justify-center items-center gap-4">
                                <button
                                    onClick={() => setStep('season')}
                                    className="px-6 py-2 text-gray-600 hover:text-gray-800 transition-colors"
                                >
